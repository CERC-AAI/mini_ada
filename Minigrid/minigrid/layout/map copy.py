    ...
    def _can_merge(self, submap_obj, start_coords):
        ...
        for r in range(len(submap)):
            for c in range(len(submap[0])):
                ...
                #    mastermap_tile = self.obj_map[row_idx][col_idx]
                #except:
                    #print('Daria: error in _can_merge()')
                    #exit()
                #    breakpoint()
                submap_tile = submap[r][c]
                #print((mastermap_tile is not None and not mastermap_tile.can_overlap_tile(submap_tile))) # todo this line added by daria
                #if mastermap_tile != None and submap_tile != None:
                #    if (mastermap_tile.tile_type == "ExitRoom" and submap_tile.tile_type == "ExitCorr") or (mastermap_tile.tile_type == "ExitCorr" and submap_tile.tile_type == "ExitRoom"):
                #        breakpoint()
